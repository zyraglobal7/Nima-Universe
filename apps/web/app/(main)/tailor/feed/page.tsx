  ...
  return (
    ...
    >
      <div className="aspect-[3/4] bg-surface-2 flex items-center justify-center overflow-hidden">
        <Scissors className="w-10 h-10 text-text-secondary opacity-30 group-hover:opacity-50 transition-opacity" />
      </div>
      <div className="p-4 space-y-1">
        <p className="font-medium text-text-primary text-sm leading-tight line-clamp-2">{item.name}</p>
...
export default function TailorFeedPage() {
  ...
  return (
    <div className="space-y-6 pb-16">
      <div>
        <h1 className="text-2xl font-serif font-semibold text-text-primary">Tailored for you</h1>
      ...
      {designs.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center space-y-3">
          ...
          <p className="text-text-secondary text-sm">No designs available yet. Check back soon.</p>
        </div>
      ) : (
